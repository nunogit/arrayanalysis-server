	...
	if(isset($_REQUEST["describe"]) && isset($_REQUEST["method"])){
	 ...
	 try{
	   $data = $this->executeMethod();
		...
		$httpCode = 200;
		if(is_integer($e->getCode()) && $e->getCode() >= 200 && $e->getCode() <= 599){
			$httpCode = $e->getCode();
		}
		$data = Array("Errors"=>array('code'=>$e->getCode(), 'message'=>$e->getMessage()));
		$this->deliver_response($_REQUEST["format"], $data, $httpCode);
	 }
 ...
 * @return void
 **/
function deliver_response($format, $api_response, $code = 200){
	...
	$method = $_REQUEST["method"]; 
        ...
        $xml_response = "<ns1:".$method."Response xmlns:ns1='http://www.wso2.org/php/xs' xmlns:ns2='http://www.wikipathways.org/webservice'  >\t". array_to_xml( $api_response, "ns1") ."\n</ns1:".$method."Response>";
