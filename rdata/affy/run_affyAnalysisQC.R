...
#and create a page indicating the naming and grouping used
   ...
   aDocname = strsplit(refName, "[/]")[[1]];
   docname = aDocname[length(aDocname)];
#   print(paste("docname is ", docname, " ", length(aDocname), " ", aDocname));
   print(paste("docname is ", docname));

  coverAndKeyPlot(description, docname, WIDTH=WIDTH,HEIGHT=HEIGHT)
...
if(clusterRaw){
  ...
  clusterFun(Data=rawData, clusterOption1=clusterOption1, 
	 clusterOption2=clusterOption2,WIDTH=WIDTH,HEIGHT=HEIGHT,
	 POINTSIZE=POINTSIZE,MAXARRAY=maxArray) 
}
...
#----------------------------------------------------
  ...
  if(clusterNorm){
    ...
    clusterFun(Data=normData, clusterOption1=clusterOption1, 
	  clusterOption2=clusterOption2, normMeth=normMeth, WIDTH=WIDTH,
	  HEIGHT=HEIGHT,POINTSIZE=POINTSIZE,MAXARRAY=maxArray)
  }
